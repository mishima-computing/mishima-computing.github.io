...
role_spec: roles/aggressive-designer.md
adapter: .claude/agents/aggressive-designer.md
schema: schemas/design-proposal.schema.json
allowed_files: none (read-only role)
forbidden_files: all writes forbidden
expected_output: .agent-runs/20260612-121929-benchmark-stage-a/carriers/claude/aggressive-designer/result.json
...
Produce a design proposal per your role contract. Read the ACTUAL SITE first: index.html, about/, contact/, docs/, kessan/, incidents/, reports/, agent-readiness/ (the real content is the design material — a thesis written without reading it is invalid). Then .agent-org/intake-template.md, .agent-org/knowledge/ui/ (all 7 cards + anchors as cited), exemplars.md. Worth questioning per your role: what is the ONE thing on this site that is genuinely unusual for a Japanese K.K. (an AI-organization company publishing incidents/, kessan/, agent-readiness/ openly) and does the thesis make THAT the memorable move instead of decorating around it; and whether the previous failure's real lesson is that corporate-trust register and boring are being conflated — what separates 誠実 from 退屈 in concrete composition terms.
Set role_id to "aggressive-designer". Return pure JSON conforming to the schema, including the required confidence object. Include ONLY the properties defined in the schema; if you drop a field, remove the key entirely. Keep the JSON compact: each string at most 400 characters, each array at most 12 items.
